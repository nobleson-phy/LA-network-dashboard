"""
Interactive Network Graph Dashboard Generator (v2)

Reads gaze activity CSV data, computes network graph data for every userId x taskId x modality combination,
and generates a standalone HTML file with embedded Cytoscape.js.

Usage:
    python generate_dashboard_v2.py <csv_file> [options]

Example:
    python generate_dashboard_v2.py all_users_gaze.csv
"""
...
def parse_arguments():
    ...
    parser.add_argument("--output", "-o", default="dashboard_interactive.html",
                        help="Output HTML file path (default: dashboard_interactive.html)")
...
def preprocess_data(csv_file):
    ...
    for old, new in column_mapping.items():
        if old in df.columns:
            df[new] = df[old]
        elif new not in df.columns and old != new:
            print(f"Warning: Column '{old}' not found, creating '{new}' as empty")
            df[new] = np.nan
    ...
    # Modality: if X/gazeX is nonzero, it's "eTrack", otherwise "mclick"
    df["X_numeric"] = pd.to_numeric(df["X"], errors="coerce")
    df["modality"] = np.where((df["X_numeric"].notna()) & (df["X_numeric"] != 0), "eTrack", "mclick")
    df = df.drop("X_numeric", axis=1)

    # Timestamp parsing - try ISO 8601 format first
    ...
    # Clean Panel_Title - keep unicode characters but remove problematic ones
    def clean_panel_title(title):
        if pd.isna(title) or title == "":
            return ""
        # Keep the title mostly as-is, just trim whitespace
    ...
    for uid in user_ids:
        ...
        for task_id in unique_tasks:
            ...
            for mod in modalities:
                ...
                # Build edge list with both time and frequency info
                edges = []
...
function updateModalityToggles(){
  ...
  // Get available modalities for current task (from first user that has this task)
  ...
  if(availableMods.length === 0) availableMods = ['mclick', 'eTrack'];
...
def main():
    ...
    df, user_ids = preprocess_data(args.csv_file)
    ...
    all_graphs, user_tasks, available_modalities = compute_all_graph_data(
        ...
        min_duration=0,  # No duration filtering - let user control
        min_frequency=1,  # Minimal frequency filtering - let user control
        edge_weight_method="total",  # Will show both time and frequency
        edge_representation="time",  # Will show both representations
    )
